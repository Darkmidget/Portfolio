/* ========================================
   ...
   🤖 AI ASSISTANT CONTEXT - FOLDER & ARCHITECTURE:
   - `src/data/projects.js` : Main hub (Profile, Skills, and project aggregator).
   - `src/data/projects/`   : Individual project files (`<id>.js`) & `manifest.json`.
   - `src/data/projects/loader.js` : Dynamically loads the manifest and all project scripts.
   - `src/js/`              : Core frontend logic (e.g., `analytics.js`).
   - `tools/`               : Node.js CLI scripts (e.g., `update-project-manifest.js`).
   - `analytics-exports/`   : Analytics artifacts generated by `.github/workflows/`.
   - `resources/Pictures/`  : Images grouped by project ID.
   *For a comprehensive breakdown, refer to the `AI_CONTEXT.md` file in the root directory.*
